...
export async function createSpaceAction(formData: FormData) {
  ...
  if (!ctx) return;
  const space = await codex.createSpace(ctx.user.id, ctx.workspace.id, createSpaceSchema.parse({ key: String(formData.get("key") || "").trim().toUpperCase().replace(/[^A-Z0-9]/g, ""), name: String(formData.get("name") || ""), description: String(formData.get("description") || "") }));
