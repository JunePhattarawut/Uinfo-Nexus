...
export async function createPageAction(spaceKey: string, spaceId: string, formData: FormData) {
  ...
  if (!ctx) return;
  const page = await codex.createPage(ctx.user.id, ctx.workspace.id, createPageSchema.parse({ spaceId, parentId: String(formData.get("parentId") || "") || null, title: String(formData.get("title") || ""), contentText: String(formData.get("contentText") || "") }));
...
export async function updatePageAction(spaceKey: string, pageId: string, formData: FormData) {
  const ctx = await actionGuard("page:edit");
  if (!ctx) return;
  await codex.updatePage(ctx.user.id, ctx.workspace.id, pageId, updatePageSchema.parse({ title: String(formData.get("title") || ""), contentText: String(formData.get("contentText") || ""), publish: formData.get("publish") === "on" }));
